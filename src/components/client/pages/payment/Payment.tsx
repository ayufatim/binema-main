...
function Payment({ movie, paymentPlan, currentUser, paymentCard, paymentPromo, allPayment }: { movie: SafeMovie, paymentPlan: SafePaymentPlan, currentUser: any, paymentCard: SafePaymentCard[], paymentPromo: SafePaymentPromo[], allPayment: SafePayment[] }) {
    ...
    useEffect(() => {
        const paymentPromoCodes = paymentPromo.map(pay => pay.promoCode);
                    ...
                    <div className='mt-5 sm:mt-10 w-fit'>
                        <form onSubmit={handleSubmit(onSubmit)}>
                            <div className='flex flex-col'>
                                <div className='flex flex-col'>
                                    {paymentCard.map((pay: any, index: number) => (
                                        <div className='flex flex-col pb-2 items-center' key={index}>
                                            <div
                                                className={`flex flex-row w-full h-20 w-[70vw] lg:w-[40vw] justify-between rounded-tr-lg rounded-tl-lg ${index === checkedIndex ? 'bg-gray-200' : 'bg-white hover:bg-gray-100 rounded-br-lg rounded-bl-lg cursor-pointer'}`}
                                            ...
                                            >
                                                ...
                                                />
                                                {index === checkedIndex
                                                    ? <div className='flex items-center mr-3 text-black text-md font-semibold'>Rp {totalPrice.toString().replace(/\B(?=(\d{3})+(?!\d))/g, ".")}</div>
                                                    : <input type="radio" className="flex mr-3" checked={index === checkedIndex} value={pay.nameCard} onChange={handleMethodPayment} required />
                                                }
                                            </div>
                                            {index === checkedIndex
                                                ?
                                                <div className='flex flex-row justify-between items-center w-full py-2 bg-gray-200 text-sm rounded-br-lg rounded-bl-lg border-t-2 border-gray-400'>
                                                    <span className='text-black pl-4 font-medium'>Bayar dengan {pay.nameCard}</span>
                                                    <a href={`/tutorial/${pay.nameCard.toLowerCase()}`} className='text-blue-500 font-bold mr-3 border-2 border-blue-500 p-1 rounded-lg cursor-pointer animate-bounce mt-2'><span>Cara Kerja?</span></a>
                                                </div>
                                                : <></>
                                            }
                                        </div>
                                ...
                                <div className='flex flex-col mt-5 bg-white text-black rounded-[25px] p-5 gap-2'>
                                    <div>
                                    ...
                                    <div className="relative">
                                        ...
                                        <input type="datetime-local" value={startTime} ref={inputRef} onChange={dateOnChange} min={now.toISOString().slice(0, 16)} className="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full pl-10 p-2.5" />
                                    </div>
                                </div>
                                <button
                                    type='submit'
                                    className='flex mt-5 text-black self-center bg-white px-10 py-2 rounded hover:text-white hover:bg-[#333] duration-300'
                                >
                                    Bayar
                                </button>
                            </div>
