...
import { SafePaymentCard } from "@/utils/types/safeData";

const PaymentMethod = ({paymentCard}: {paymentCard: SafePaymentCard[]}) => {
        ...
        <div className="pt-5 items-center justify-center justify-items-center pb-5">
            <div className="mx-auto mt-8 max-w-screen-lg px-2">
                <div className="sm:flex sm:items-center sm:justify-between flex-col sm:flex-row">
                    <p className="flex-1 text-base font-bold text-white">List Card</p>
                    ...
                    <div className="mt-4 sm:mt-0">
                        <div className="flex items-center justify-start sm:justify-end">
                            ...
                            <a href="/admin/payment/paymentMethod/add" className="inline-flex cursor-pointer items-center rounded-lg border border-gray-400 bg-white py-2 px-3 text-center text-sm font-medium text-black shadow hover:bg-[#3B8AE5] hover:text-white duration-300 focus:shadow">
                ...
                <div className="mt-6 overflow-x-auto rounded-xl border shadow">
                    <table className="min-w-full border-separate border-spacing-y-2 border-spacing-x-2">
                        <thead className="border-b lg:table-header-group">
                            <tr className="">
                                <td className="whitespace-normal py-4 text-sm font-medium text-white px-6">Jenis</td>
                                <td className="whitespace-normal py-4 text-sm font-medium text-white px-6">Nomor</td>
                                <td className="whitespace-normal py-4 text-sm font-medium text-white px-6">Kategori</td>
                                <td className="whitespace-normal py-4 text-sm font-medium text-white px-6">Gambar Card</td>
                                <td className="whitespace-normal py-4 text-sm font-medium text-white px-6">QR</td>
                                <td className="whitespace-normal py-4 text-sm font-medium text-white px-6">Aksi</td>
                            </tr>
                        </thead>
                        <tbody className="lg:border-gray-300">
                            {paymentCard.map((payCard: any, index: any) => (
                                <tr className="" key={index}>
                                    ...
                                    <td className="whitespace-no-wrap py-4 text-sm font-normal text-white px-6 table-cell">
                                        {payCard.categoryInstitue}
                                    </td>
                                    <td className="whitespace-no-wrap py-4 px-6 text-left text-sm text-white">
                                        <img src={payCard.imageCard} alt={payCard.nameCard} width={100}/>
                                    </td>
                                    {payCard.imageQR == null || '' ? <div>Gambar tidak ada</div> : 
                                        <td className="whitespace-no-wrap py-4 px-6 text-left text-sm text-white">
                                            <img src={payCard.imageQR} alt={payCard.nameCard} width={100}/>
                                        </td>
                                    }
                                    <td className="whitespace-no-wrap py-4 text-sm font-normal text-white px-6 lg:table-cell flex flex-row gap-5">
                                        <Link className='bg-green-600 px-5 py-2 rounded-lg flex flex-row gap-2 items-center justify-center sm:mb-2' href={`/admin/payment/paymentMethod/${payCard.id}/edit`}>
                                        ...
                                        <Link className='bg-red-600 px-5 py-2 rounded-lg flex flex-row gap-2 items-center justify-center' href={'#'}>
                                            <button className='flex flex-row gap-2 items-center justify-center' onClick={() => handleClickDelete(payCard.id)}>
